    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.seq_last:
            input = input.transpose(
                -1, 1)  # [B, H, Seq] -> [B, Seq, H], or [B,H,w,h] -> [B,h,w,H]
...
class GlobalLayerNorm(nn.Module):
    ...
    def extra_repr(self) -> str:
        return '{dim_hidden}, seq_last={seq_last}, eps={eps}'.format(
            **self.__dict__)
...
class GroupBatchNorm(Module):
    ...
    def __init__(
        ...
        assert (dims_norm is not None
                and dim_affine is not None) or (dims_norm is not None), (
                    dims_norm, dim_affine, 'should be none at the time')
    ...
    def forward(self, x: Tensor, group_size: int = None) -> Tensor:
        ...
        if self.dims_norm is not None:
            ...
            if x.ndim == 4:
                assert group_size is None or group_size == x.shape[1], (
                    group_size, x.shape)
...
def new_norm(norm_type: str,
    ...
    if norm_type.upper() == 'LN':
        ...
        norm = GroupNorm(num_groups=num_groups,
