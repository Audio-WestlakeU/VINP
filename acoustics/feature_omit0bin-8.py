...
class transforms:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # for i in range(1, osFac):
        #     coe = coe + win[i * hop_len : (i + 1) * hop_len] ** 2
        # coe = coe.repeat(osFac)
        # self.awin = win / torch.sqrt(coe)
        # self.swin = self.awin/ n_fft/ torch.sqrt(coe)
    ...
    def preprocess(self, input: torch.Tensor) -> torch.Tensor:
        ...
        """
        # output = input.pow(1 / 3)
        output = (input+1e-8).log10()
        # output = (input + 1e-32).log()
        # output = input
    ...
    def postprocess(self, input: torch.Tensor) -> torch.Tensor:
        ...
        """
        # output = input.pow(3).clamp(0)
        output = 10**(input.clamp(-8,6))
        # output = input.exp()
        # output = input
        return output.contiguous()

    def preprocess_log(self, input: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
        """
        输入网络前的处理，包括幅度变换、维度变换等
        """

        output = (input + eps).log()

        return output

    def postprocess_log(self, input: torch.Tensor) -> torch.Tensor:
        """
        网络输出后的处理，包括幅度变换、维度变换等
        """

        output = torch.clamp(input, min=-16, max=16).exp()

        return output

    def preprocess_FSN(self, input: torch.Tensor) -> torch.Tensor:
        output = input.pow(1 / 3)
        return output

    def postprocess_FSN(self, input: torch.Tensor) -> torch.Tensor:
        output = input.pow(3).clamp(0)
        return output

    def preprocess_normAbs(self, input: torch.Tensor) -> torch.Tensor:
        """
        输入网络前的处理，包括幅度变换、维度变换等
        """
        output = (input + 1e-16).log()

        return output.contiguous()

    def postprocess_normAbs(self, input: torch.Tensor) -> torch.Tensor:
        """
        网络输出后的处理，包括幅度变换、维度变换等
        """

        output = torch.clamp(input, -512, 512).exp()
        return output.contiguous()

    def pad_spec(self, input):
        T = input.size(-1)
        if T%64 !=0:
            num_pad = 64-T%64
        else:
        ...
        input_padded = pad2d(input)
        return input_padded,num_pad
    
    def preprocess_UNET(self, input: torch.Tensor) -> torch.Tensor:

        # max_val = input.abs().max().item()
        # input = input/max_val
        input,num_pad = self.pad_spec(input)
        
        # output = (input.clamp(1e-32)).log10()
        # output = input
        output = (input.abs()+1e-32).log10()+(input.abs()+1e-32).log10()*1j

        return output.contiguous(),num_pad
    
    def postprocess_UNET(self, input: torch.Tensor,num_pad:int) -> torch.Tensor:

        # output = 10**input.clamp(-32,32)
        # output = input
        output = 10**(input.real.clamp(-32))
        output = output[...,:output.shape[-1]-num_pad]
