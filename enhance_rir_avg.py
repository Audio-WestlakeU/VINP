...
def enhance_avg(
    ...
    **kwargs
    ...
    for fpath_input_n in tqdm(fpath_input):
        ...
        if ref_path:
            fpath_ref_n=os.path.join(ref_path,basename_input_n)
            ref_wav=load_wav(fpath_ref_n, sr)
            ref_wav/=scale
            ref_wav=torch.nn.functional.pad(ref_wav,(0,input_wav.shape[-1]-ref_wav.shape[-1]))
        else:
            ref_wav=None
        ...
        spch_est, rir, steps, likelihood = rkem.process(input_wav, mymodel, TF, device,ref_wav)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--ref_path", required=False, default=None, help="reference path"
    )
