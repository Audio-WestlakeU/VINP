...
from networks.transducer.beam_search_transducer import BeamSearchTransducer
from networks.transducer.beam_search_transducer import (
    ExtendedHypothesis as ExtTransHypothesis,
)
from networks.transducer.beam_search_transducer import Hypothesis as TransHypothesis
from tasks.asr_enh import ASRTask
from tasks.lm import LMTask

from text.build_tokenizer import build_tokenizer
from text.token_id_converter import TokenIDConverter
from text.whisper_token_id_converter import OpenAIWhisperTokenIDConverter
...
from utils.custom_types import str2bool, str2triple_str, str_or_none
from search.batch_beam_search import BatchBeamSearch
from search.batch_beam_search_online_sim import BatchBeamSearchOnlineSim
from search.beam_search import BeamSearch, Hypothesis
from search.beam_search_timesync import BeamSearchTimeSync
from networks.subsampling import TooShortUttError
from search.scorer_interface import BatchScorerInterface
from search.scorers.ctc import CTCPrefixScorer
from search.scorers.length_bonus import LengthBonus
from utils.cli_utils import get_commandline_args
from networks.model_summary import model_summary

from torch.utils.data import DataLoader
from tasks.fileio.datadir_writer import DatadirWriter
from tasks.preprocessor import CommonPreprocessor
from tasks.collate_fn import CommonCollateFn
from tasks.iterable_dataset import IterableESPnetDataset
...
class Speech2Text:
    ...
    def __init__(
        ...
        if ngram_file is not None:
            if ngram_scorer == "full":
                from search.scorers.ngram import NgramFullScorer

                ngram = NgramFullScorer(ngram_file, token_list)
            else:
                from search.scorers.ngram import NgramPartScorer
    ...
    def __call__(
        ...
        # b. Forward Encoder
        enc, enclen = self.asr_model.encode(name, **batch)
...
def inference(
    ...
    retval = CommonPreprocessor(
        ...
        token_list=speech2text.asr_train_args.token_list,
        bpemodel=speech2text.asr_train_args.bpemodel,
        non_linguistic_symbols=speech2text.asr_train_args.non_linguistic_symbols,
        text_cleaner=speech2text.asr_train_args.cleaner,
        speech_volume_normalize=speech2text.asr_train_args.speech_volume_normalize
        if hasattr(speech2text.asr_train_args, "rir_scp")
        else None,
    )

    collate_fn=CommonCollateFn(float_pad_value=0.0, int_pad_value=-1)
    if collate_fn is not None:
        loaderkwargs = dict(collate_fn=collate_fn)
    else:
        loaderkwargs = {}
    ...
    if dataset.apply_utt2category:
        loaderkwargs.update(batch_size=1)
    else:
        loaderkwargs.update(batch_size=batch_size)
