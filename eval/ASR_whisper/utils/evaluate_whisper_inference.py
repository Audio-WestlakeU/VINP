...
class Speech2Text:
    ...
    def __call__(self, speech: str, **decode_options) -> Optional[str]:
        ...
        #     start_ms = int(ts['start'])  # 转换为毫秒
        #     end_ms = int(ts['end'])
        #     speech_chunk = wav[...,start_ms:end_ms]
        #     combined.append(speech_chunk)
        # combined=np.concatenate(combined,-1)        

        # result = whisper.transcribe(self.model,combined, **decode_options)
        result = whisper.transcribe(self.model,speech, **decode_options)
